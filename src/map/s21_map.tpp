...
template <typename Key, typename Value> map<Key, Value>::map(const map &m) {
  for (ConstIterator it = m.cbegin(); it != m.cend(); ++it) {
    insert((*it).first, (*it).second);
  }
}

template <typename Key, typename Value> map<Key, Value>::map(map &&m) noexcept {
  tree.swap(m.tree);
}
...
template <typename Key, typename Value>
typename map<Key, Value>::mapped_type &
map<Key, Value>::operator[](const key_type &key) {
...
template <typename Key, typename Value>
[[nodiscard]] typename map<Key, Value>::size_type
...
template <typename Key, typename Value>
std::pair<typename map<Key, Value>::iterator, bool>
map<Key, Value>::insert(const key_type &key, const mapped_type &value) {
  auto node = tree.get(key);
