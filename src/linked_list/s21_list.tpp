...
template <typename T> list<T>::list(std::initializer_list<T> const &items) {
  size_ = 0;
  for (auto item : items) {
    push_back(item);
  }
...
template <typename T> void list<T>::mergeSort(list<T> &other) {
  if (other.size_ < 2)
    return;
...
template <typename T> void list<T>::sort() { mergeSort(*this); }
